...
module case(tentA=0,right=0,trrs=1,switch=0,printedPlate=0,fill=1) {
    bottomW = 2.5;
    sideW = 4;
    edgeH = 4.5;
    overH = printedPlate == 1 ? 0.7 : 2.5;
    delta = 0.5;
    ...
    difference() {
        ...
        translate([-62.25, 78.2,-3]) {
            ...
            translate(right==0 ? [0,27,0] : [0,27,-0.5])
                hull() {
...
module caseWithLipo(right=0, switch=1, trrs=0) {
...
module caseWithLipoExt(right=0, switch=1) {
    difference() {
        case(right=right, trrs=0, switch=switch){
